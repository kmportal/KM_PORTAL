	...
	if(document.forms[0].briefingHeading.value=="")
	{
		...
		while(i<=count1){
			if(document.getElementById("briefingDetails["+i+"]").value==""){
				...
				return false;	
			}else if(!reg.test(document.getElementById("briefingDetails["+i+"]").value))
		{
		alert("Please remove quote symbols from message box"+(i+1));
		return false;	
	   }
...
<html:form action="/kmBriefingMstr">
	...
	<TABLE align="center"> 
		...
		<li class="clearfix alt">
			<span class="text2 fll width160"><strong><bean:message key="briefing.displayDate" /></strong></span>
		...
		<li class="clearfix">
			...
			<TABLE border="0" align="left"  width="50%" id="briefingTable">						
				<c:forEach begin="0" end="${count}" var="i" step="1" >
					<TR>
					  <TD align="left" >
						<html:textarea  styleClass="textarea2"  onkeypress =  "return killEnter(event);" onkeydown="return limitText(this);" property='<%="briefingDetails[" + ((Integer)(pageContext.getAttribute("i"))).intValue()+ "]"%>' styleId='<%="briefingDetails[" + ((Integer)(pageContext.getAttribute("i"))).intValue()+ "]"%>' /></TD>
					</TR>
				</c:forEach>
			</TABLE>
		</li>
