...
public class ApproveFileDaoImpl implements ApproveFileDao{
	...
	Logger logger = Logger.getLogger(ApproveFileDaoImpl.class);
...
//KM Phase 2
	...
	public ArrayList getFileList(String userId)
		throws KmException {
			...
			logger.info("Entered in getFileList method ::" + userId);
			try
			{
				con = getConnection();
				pstmt = con.prepareStatement(query.append(" with ur ").toString());
				logger.info("Final Query for Files to be approved "+query);
				pstmt.setString(1, "1");
	...
	public void rejectFile(String[] fileIds,String[] commentList,String userId) throws KmException {
				...
				Connection con = null;
				PreparedStatement pstmt = null;
				logger.info("Entered in  rejectFile method" +fileIds+" ### "+commentList+" $$$ "+userId);
				try
				{
					...
					while( i < fileIds.length)
					{
						logger.info(commentList[i]+" %%% Comments ");
						pstmt.setString(1,commentList[i]);
						pstmt.setInt(2,Integer.parseInt(userId));
						pstmt.setInt(3,Integer.parseInt(fileIds[i]));
						pstmt.addBatch();
