...
public class KmSearchDaoImpl implements KmSearchDao {
	...
	protected static final String GET_SNB_DETAILS = "SELECT SRNO, SCHEME_TYPE , DESCRIPTION , DISPLAY_FLAG FROM KM2.KM_SCHEMESANDBENEFITS WHERE ";
	...
	public ArrayList contentSearch(KmSearch searchDto, String[] documentIds)
		...
		try {
			...
			sql
			.append(") AND doc.STATUS ='A' ");
			if (searchDto.getSearchType().equals("CSR_CONTENT")) {
				sql.append(" AND doc.APPROVAL_STATUS='A' ");
			}
			
			
			logger.info("Search Dto getSerachType    -----    "+searchDto.getRcCategoryId());
			
			if(searchDto.getRcCategoryId()!=null){
				logger.info("Inside RC search");
			}
			else{
				sql.append("AND (DATE(CURRENT TIMESTAMP) BETWEEN DATE(PUBLISHING_START_DT) AND DATE(PUBLISHING_END_DT)) ");
			}

			
			logger.info("Final Query For RC Data "+sql);
			logger.info("Our search dto document name      -----        "+searchDto.getDateCheck());
			...
			logger.info("The size of filelist returned by KmSearchDaoImpl       ------          "+fileList.size());
			return fileList;
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public ArrayList<KmSearchDetailsDTO> searchDetails(String keyword,
		...
		try {
			if (mainOptionValue.equalsIgnoreCase("arc")) {
				...
				query = query + " LOWER(TRIM(VASNAME)) LIKE '%"
				+ keyword.toLowerCase() + "%' WITH UR";
			}
			//Added by Saanya for schemes and benefits
			else if (mainOptionValue.equalsIgnoreCase("snb")) {
				query = GET_SNB_DETAILS;
				logger.debug("GET_SNB_DETAILS :: " + GET_SNB_DETAILS);
				query = query + " LOWER(TRIM(SCHEME_TYPE)) LIKE '%"
				+ keyword.toLowerCase() + "%' WITH UR";
			}
			...
			while (rs.next()) {
				...
				//Added by Saanya for schemes and benefits
					...
					searchDetailsDTO.setSrNo(rs.getInt("SRNO"));
					searchDetailsDTO.setSchemeType(rs.getString("SCHEME_TYPE"));
					searchDetailsDTO.setSchemeDesc(rs.getString("DESCRIPTION"));
					searchDetailsDTO.setSchemeDisplayFlag(rs.getString("DISPLAY_FLAG"));
				}
	...
	public Map<String, Object> sendSms(String mainOption, int serialNo)
		...
		try {
			...
			/*Added by Saanya for Self Care */
			else if (mainOption.equalsIgnoreCase("snb"))
				tableName = "KM_SCHEMESANDBENEFITS";
			...
			if (configurableColumnList != null) {
				...
				String baseQuery;
				if (mainOption.equalsIgnoreCase("snb")){
					baseQuery = "select " + sb1 + " from " + tableName
					+ " where SRNO =  ?" ;
				}
				else{
					baseQuery = "select " + sb1 + " from " + tableName
