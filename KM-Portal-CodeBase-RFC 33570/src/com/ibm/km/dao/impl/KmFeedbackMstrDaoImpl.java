...
public class KmFeedbackMstrDaoImpl implements KmFeedbackMstrDao{
	...
	protected static final String SQL_SELECT_FEEDBACKS ="WITH nee(element_id,chain,element_level_id,element_name) AS  ( SELECT  ELEMENT_ID,  cast(element_name as VARCHAR(" + PropertyReader.getAppValue("search.path.size") + ")) ,element_level_id,element_name  FROM KM_ELEMENT_MSTR  WHERE element_id = ?  UNION ALL   SELECT nplus1.element_id,nee.chain || '/' || nplus1.element_name,nplus1.element_level_id,nplus1.element_name  FROM KM_ELEMENT_MSTR as nplus1, nee  WHERE nee.element_id=nplus1.PARENT_ID) SELECT  fed.FEEDBACK_ID, chain,fed.COMMENT, fed.ELEMENT_ID, nee.ELEMENT_NAME, fed.CREATED_BY, fed.CREATED_DT, usr.USER_LOGIN_ID, fed.OLM_ID FROM KM_FEEDBACK_MSTR fed inner join NEE  on fed.ELEMENT_ID=nee.ELEMENT_ID inner join KM_USER_MSTR usr on usr.USER_ID=fed.CREATED_BY WHERE  fed.READ_STATUS='N'  ORDER BY fed.CREATED_DT  desc ";
	
   
   protected static final String SQL_FEEDBACK_RESP="SELECT COMMENT , CREATED_DT, UPDATED_DT,READ_STATUS,FEEDBACK_RESP, OLM_ID FROM KM_FEEDBACK_MSTR WHERE CREATED_BY=?   ORDER BY CREATED_DT DESC ";
   
   protected static final String SQL_FEEDBACK_REPORT ="WITH nee(element_id,chain,element_level_id,element_name) AS  ( SELECT  ELEMENT_ID,  cast(element_name as VARCHAR(" + PropertyReader.getAppValue("search.path.size") + ")) ,element_level_id,element_name  FROM KM_ELEMENT_MSTR  WHERE element_id = ?  UNION ALL   SELECT nplus1.element_id,nee.chain || '/' || nplus1.element_name,nplus1.element_level_id,nplus1.element_name FROM KM_ELEMENT_MSTR as nplus1, nee  WHERE nee.element_id=nplus1.PARENT_ID) SELECT fed.CREATED_DT,fed.UPDATED_DT,usr.USER_LOGIN_ID, fed.COMMENT, fed.READ_STATUS, fed.OLM_ID FROM KM_FEEDBACK_MSTR fed inner join NEE  on fed.ELEMENT_ID=nee.ELEMENT_ID inner join KM_USER_MSTR usr on usr.USER_ID=fed.CREATED_BY WHERE fed.READ_STATUS !='N'   and date(fed.CREATED_DT) between DATE(timestamp_format(?, 'YYYY-MM-DD HH24:MI:SS')) and DATE(timestamp_format(?, 'YYYY-MM-DD HH24:MI:SS'))  ORDER BY fed.CREATED_DT";
	/* (non-Javadoc)
	 * @see com.ibm.km.dao.KmFeedbackMstrDao#insert(com.ibm.km.dto.KmFeedbackMstr)
	 * 
	 */
   protected static final String SQL_INSERT_WITH_OLMID ="INSERT INTO KM_FEEDBACK_MSTR (FEEDBACK_ID, COMMENT, CREATED_DT, CREATED_BY, CIRCLE_ID, CATEGORY_ID, SUB_CATEGORY_ID, ELEMENT_ID, READ_STATUS, OLM_ID) VALUES ( NEXTVAL FOR KM_FEEDBACK_ID,?, current timestamp, ?, ?, 0, 0,?,'N', ?)";
   
   protected static final String SQL_FEEDBACK_OLMID="Select OLM_ID from KM_USER_MSTR where USER_LOGIN_ID= ?";
	public void insert(KmFeedbackMstr dto) throws KmException {
		...
		/**	ps.setString(paramCount++, dto.getComment());
			...
			//For OLM Id in Feedback
			
			StringBuffer query=new StringBuffer(SQL_INSERT_WITH_OLMID);
			//String sql = SQL_INSERT_WITH_OLM_ID;
			con = DBConnection.getDBConnection();
			logger.info("Created by:" + dto.getCreatedBy() + " CircleID:" + dto.getCircleId() + " ElementID:" + dto.getElementId());
			ps = con.prepareStatement(query.append(" with ur").toString());
			int paramCount = 1;
	...
	public ArrayList viewFeedbacks(String[] elementIds, String elementId) throws KmException {
		...
		/*	for(int i=0;i<elementIds.length;i++){
			...
			while(rs.next()){
				...
				dto.setOlm_id(rs.getString("OLM_ID"));
	...
	public ArrayList feedbackResponseAll(int userId) throws DAOException {
		...
		String sql = "SELECT COMMENT , CREATED_DT, UPDATED_DT,READ_STATUS,FEEDBACK_RESP, OLM_ID FROM KM_FEEDBACK_MSTR WHERE CIRCLE_ID=? order by CREATED_DT desc" ;
		try
		{
			con = DBConnection.getDBConnection();
			...
			while(rs.next()) {
				...
				//Olm Id in Feedback
				feedbackDto.setOlm_id(rs.getString("OLM_ID"));
				//OLM id in feedback
	...
	public void getOLMIDFor(String userLoginId)throws KmException{
		...
		KmFeedbackMstr dto=null;
		dto=new KmFeedbackMstr();
		StringBuffer query=new StringBuffer(SQL_FEEDBACK_OLMID);
		try{
		con = DBConnection.getDBConnection();
		ps = con.prepareStatement(query.append(" with ur").toString());
		...
		logger.info("OLM ID @@@ "+dto.getOlm_id());
